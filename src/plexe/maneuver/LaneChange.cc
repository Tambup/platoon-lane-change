...
bool LaneChange::initializeLaneChangeManeuver(const void* parameters)
{
    LaneChangeParameters* pars = (LaneChangeParameters*) parameters;
    if (laneChangeManeuverState == LaneChangeManeuverState::IDLE) {
        if (app->isInManeuver()) {
        ...
        laneChangeManeuverState = LaneChangeManeuverState::J_WAIT_REPLY;
...
{
    if (initializeLaneChangeManeuver(parameters)) {
        // send join request to leader
        LOG << positionHelper->getId() << " sending ChangeLane to platoon with id " << targetPlatoonData->platoonId << " (leader id " << targetPlatoonData->platoonLeader << ")\n";
...
bool LaneChange::processJoinRequest(const JoinPlatoonRequest* msg)
{
...
void LaneChange::handleJoinPlatoonRequest(const JoinPlatoonRequest* msg)
{
...
void LaneChange::handleMergePlatoonRequest(const MergePlatoonRequest* msg)
{
    // do nothing. This implementation is meant for single joining cars
}

void LaneChange::handleJoinPlatoonResponse(const JoinPlatoonResponse* msg)
{
...
void LaneChange::handleMoveToPosition(const MoveToPosition* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
...
void LaneChange::handleMoveToPositionAck(const MoveToPositionAck* msg)
{
...
void LaneChange::handleJoinFormation(const JoinFormation* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::JOINER) return;
...
void LaneChange::handleJoinFormationAck(const JoinFormationAck* msg)
{
    if (app->getPlatoonRole() != PlatoonRole::LEADER) return;
